...
B.05 - Abra o JSON recebido no visualizador de provas (`index.html`) e confirme manualmente que esse arquivo está de acordo como o esperado. Em caso positivo, responda com "Continue", e caso o LLM se recuse por causa de limitações no tamanho do JSON, diga para ler as regras em "processamentoIntegral" novamente.
...
> [!IMPORTANT]
> As alterações nos arquivos deste projetos não são refletidas automaticamente no `index.html`, então caso este arquivo seja alterado, atualize a página do navegador. Após qualquer alteração nos arquivos JSON, como a adição de imagem a uma questão, navegue para a questão anterior ou posterior e volte para a questão atual, pois tal ação força o navegador a ler novamente o arquivo JSON local, o que permite garantir que os dados, por exemplo uma imagem, tenha sido de fato persistido.
